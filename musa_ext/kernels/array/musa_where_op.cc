...
class MusaWhereOp : public MusaOpKernel {
 public:
  ...
  void ComputeType(OpKernelContext* context, const Tensor& input,
    ...
    // We first need to compute the number of true elements in the input tensor,
    // which determines the output shape.
    ...
    // Next is to use scan to compute the output indices for true elements
...
#define HANDLE_DIM(NDIM)                                                  \
  case NDIM: {                                                            \
    Status where_status = Where::Compute<NDIM, T, int64>(                 \
        context, input.tensor<T, NDIM>(), output->matrix<int64>(), marks_t); \
