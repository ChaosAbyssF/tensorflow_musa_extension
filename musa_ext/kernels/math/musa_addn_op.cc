...
void AddNCompute(OpKernelContext* ctx, mFormat format,
  ...
  if (all_shapes_match) {
    ...
    MUSA_KERNEL_TRACE_START("Mem Alloc");
    // musaMalloc returns musaError_t usually, need to check wrapper in
    // utils_op.h or mu headers If musaMalloc is the raw C API, it returns
    // musaError_t. The previous error said: cannot convert 'musaError_t' to
    // 'mStatus' in initialization. This means musaMalloc returns musaError_t.
