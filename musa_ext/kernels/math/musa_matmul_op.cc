 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (in0.dims() == 2 && in1.dims() == 2) {
      ...
      // 定义 Reshape 逻辑：仅修改 mTensor (mudnn的视图)，不触及 TF Tensor
      // 的实际内存布局
      auto ReshapeTo3D = [out_batch](mTensor& mt, const Tensor& t) {
        ...
        if (dims != 3) {
          if (batch == 1 && out_batch > 1) {
            // 处理简单的 1-to-N 广播 (例如 [1, M, K] 与 [B, K, N] 乘)，设置
            // batch_stride 为 0 以复用内存
      ...
      // 我们在此只需将底层 mt_out 视角映射为 3
      // 维即可，乘法完成后无需任何“恢复”代码， 因为 TF 侧的 out
      // 依然保持高维属性。
      if (out_shape.dims() > 3) {
