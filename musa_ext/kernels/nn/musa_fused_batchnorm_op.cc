 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (is_training_) {
      ...
      // TF 2.15: tsl::Status is [[nodiscard]]; previously we silently dropped
      // the Status from allocate_temp(), which would proceed with an
      // uninitialized Tensor on allocator OOM.  Propagate failure via
      // OP_REQUIRES_OK so the kernel reports it cleanly instead.
      OP_REQUIRES_OK(
          ctx, ctx->allocate_temp(DT_FLOAT, scale.shape(), &temp_acc_mean));
      OP_REQUIRES_OK(
          ctx, ctx->allocate_temp(DT_FLOAT, scale.shape(), &temp_acc_var));
