...
class MusaSigmoidCalibrationOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    useKernel(ctx, input, scale, output);
  }
 ...
 private:
  ...
  void useMudnn(OpKernelContext* ctx, const Tensor& input, const Tensor& scale,
    ...
    OP_REQUIRES_OK(
        ctx, ctx->allocate_temp(input.dtype(), input.shape(), &sigmoid_tensor));
    ...
    Tensor one_tensor;
    OP_REQUIRES_OK(
        ctx, ctx->allocate_temp(input.dtype(), input.shape(), &one_tensor));
    ...
    MTOP_CHECK_OK(mul_op.SetMode(::musa::dnn::Binary::Mode::MUL), "Set Mul",
                  ctx);
    MTOP_CHECK_OK_RUN(mul_op.Run(handle, scale_times_one_minus_s_mt, scale_mt,
                                 one_minus_s_mt),
